...
def test_style_knobs_stamped_through_endpoint():
    ...
    styled = {**data, "track_width_pt": 3.4, "track_halo": 0.2,
              ...
              "soft_light": 0.35, "haze_strength": 0.2}
    ...
    # the Looks knobs (v1.13) ride the same road: server default 0.0, so a client
    # that never sends them keeps the plain look, and a sent value lands on the spec
    assert spec.soft_light == 0.35 and spec.haze_strength == 0.2
    ...
    assert c.post("/api/proof", data={**data, "soft_light": 1.5}).status_code == 422
    assert c.post("/api/proof", data={**data, "haze_strength": -0.2}).status_code == 422
