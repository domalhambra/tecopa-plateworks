...
| An order plate's grid follows the print, and every grid is fetched at its own cell size: 10/30/60 m from static tiles, anything else from the 3DEP dynamic service. `DEM_RES_CHOICES` stays 10/30/60. | Plates hold about 1.44x the print's pixels on dynamic grids, and up to 4x that on the static 10/30 m bands -- chosen because static tiles are more reliable than the dynamic WMS for a mid-size grid. Fetching a finer layer and averaging it onto a coarser grid in slices was built and removed: its seam mask nulled whole slices away from the UTM central meridian (EPSG:5070 is equal-area, UTM is not), and its slice planner understated memory 4-6x. | Plan 2026-09-24-order-plate-per-order, decision 1 |
| Sub-10 m grids come from the 3DEP dynamic service: PASS by eye, at the Tecopa probe bbox -116.27, 35.88, -116.24, 35.91. Laplacian ratio 1.54. It shows sharp washes, roads and the crater rim, against a soft, stepped 10 m grid. The 0.29 NaN share seen on the dynamic grid was a probe artifact: the same 0.29 appeared on the static grid after reprojection. | The dynamic service resamples from whatever it holds. The probe checked whether it actually carries real lidar detail below 10 m before the pipeline trusted it. | Plan Task 1 |
| Coverage is measured per layer before use. Tecopa has 1 m lidar and no 3 m layer. | The dynamic service fills gaps silently. | Plan Task 4 |
| Order plate build times on the Mac, 2026-09-24. Home compact (Emigrant Pass, Tecopa): 1.5 m grid from 1 m lidar, 6.0 x 9.1 km frame, 187 MB DEM, 104 s first build, 42 s rebuilt with a warm cache. North-south (Whitney to Mono): 30 m static grid, 133 x 200 km frame, 205 MB, 286 s. East-west (Reno to Salt Lake, landscape): 210 m dynamic grid, EPSG:5070, 1138 x 759 km frame, 187 MB, 234 s. About 6% of the east-west plate is Pacific with no US data. Land cover now bakes at the DEM's own grid instead of a fixed 60 m fetch: before that change, the east-west build took 874 s and lost land cover. USGS services (the 3DEP WMS, NLCD) failed intermittently all day, sometimes for over a minute. | The spec's target was under 10 minutes. All three came in well under it once land cover followed the grid. | Plan Task 9 |
| A dynamic grid's holes are filled from the static 3DEP tiles (30 m from a 30 m grid up, else 10 m), block by block after the slices land and before the overviews. `sources.json` records the fill as its own source (role `hole fill`, layer, share filled). Prepare then measures every built plate's no-data share, and the frame's, against the 3DEP index. Measured on a copy of the holed east-west plate: 12.2% NaN before, 2.7% after, 9.5% filled, in 231 s. The 2.7% left is open Pacific past the tiles' extent, in the plate's southwest corner, outside the frame. Hillshade at 1:1 shows no seam at the fill edges. | The dynamic service can answer a sub-request with an empty tile and say nothing: the east-west plate had rectangular NaN blocks over Idaho, Montana and southern California and Nevada. `us_share` comes from index footprints, not from the data, so nothing caught it. The fill is not the removed finer-layer design: it changes NaN cells only, on the one shared grid, with no seam mask. It reads the tiles' overviews and keeps 1.5 source cells per grid cell: a full-resolution WarpedVRT read was 7x slower over the same ground, and the network ran at about 1.8 MB/s. Both the dynamic service and the static tiles return the Pacific as elevation 0, not NaN, so ocean is 0 across the filled plate. The check's expected share (1 - `us_share`) counts that ocean, so a hole up to the ocean share can pass the plate check on a coastal plate. The frame check narrows it with the frame's own coverage. | Dom, Option B, 2026-09-24 |
| The 18×24 Print goes to a photo lab. The Canon PRO-1100 prints every size up to 17 in wide: 12×18 on 13×19, 8×10 on 8.5×11, and 16×20 on 17×22 as an option. | The PRO-1100 feeds at most 17 in wide, so it cannot print 18×24. The lab's file spec (size, colour, bleed) is still open and belongs to sub-project 4. | Dom, 2026-09-24 |
