...
- `.venv/bin/python -m pytest -n auto -q` is the full suite. The 2026-09-01 run took 3 minutes 10 seconds on this Mac and failed eight tests. Seven were font-metric tests, across labels, bleed, oblique and the base cache, because Georgia is installed and the thresholds match DejaVu. The eighth was an MP4 test, because the bundled ffmpeg writes no `colr` box on macOS. CI is green on Ubuntu. Compare the failure set, not the totals. The seven are named in `changing-things.md` under Run the tests, as enumerated on 2026-08-13.
