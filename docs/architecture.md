...
| `app/orderprep.py` | Prepare: reuse or build the order's plate through `.venv-prep` subprocesses, measure a built plate's holes (`check_holes`), write state and the report. `scripts/order.py` is its command line. | `tests/test_orderprep.py` |
...
| `scripts/render_asset_farm.py` | The marketing asset farm: poster, editions, wallpapers, film, detail crop, mockups, model, coin, relief. Stamps the `terrain` record. | `tests/test_asset_farm_detail.py`, `tests/test_asset_farm_frame.py`, `tests/test_asset_farm_gate.py`, `tests/test_terrain_provenance.py`, `tests/test_export_relief.py` |
...
| `scripts/export_relief.py` | The Ghost landing page's relief-viewer images: surface and height map for each coin and the hero poster, plus the wall and close-up stills. The coin uses the GLB's own crop and height field. Gated by `build_deploy.terrain_guard`. Writes `<region>/relief/`. | `tests/test_export_relief.py` |
