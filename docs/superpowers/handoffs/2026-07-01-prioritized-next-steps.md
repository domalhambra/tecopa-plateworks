...
> **Progress (rolling):** PR #4 landed P0-a, P0-b, the `/readyz` half of P0-c, and most of P0-e. A follow-up PR then landed **P0-d** and the **V1-8 + V1-11 halves of P0-f** (lifecycle + logging). Suite is now **111 passed / 3 skipped**. What's **still open**: real 3DEP DEMs + look re-validation (P0-c — network available, not yet run), `V1-12` source-archival, and the **V1-10 finished-poster/print-correctness** half of P0-f. The quality-bar agreement with Dom below is also still open.
...
- [~] **P0-c · Real DEMs for both regions + a `/readyz` bounds/DEM-present check.** *Done:* `/readyz` endpoint + `Region.readiness()` — every region must have a present DEM whose bounds + CRS match its `region.json` (503 with a per-region report otherwise); this catches both susanville's missing DEM and the bounds-overhang drift. *Still open:* build/restore **real** 3DEP DEMs for both regions (needs network) and re-verify the by-eye look on real terrain. *Roadmap: V1-2.*
...
- [~] **P0-f · Hygiene + finish.** *Done (V1-8 lifecycle):* the sync final now routes through the blob seam (no more `final_*.png` in `region.dir`); TTL/eviction for blobs, the job registry, and `uploads/` photo dirs (`TRAILPRINT_TTL_SECONDS`, default 24h; `LocalBlobs.delete`/`sweep`). *Done (V1-11 observability):* `app/logconfig.py` (text default, JSON opt-in) — `traceback.print_exc()` replaced by `logger.exception`; job-lifecycle + upload/proof/final render logs with timings. *Still open (V1-10):* a real title/legend/margin + print-correct output (embedded DPI is in; add sRGB/gamut, bleed/trim, a finished title block — decide the look with Dom). *Roadmap: V1-8, V1-11, V1-10.*
