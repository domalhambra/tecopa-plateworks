...
> **Progress — branch `claude/prioritized-next-steps-review-a433e5` (this PR):** landed the code-side of P0-a, P0-b, the `/readyz` half of P0-c, and most of P0-e. Test suite went from 52 passed / 24 skipped → **93 passed / 3 skipped** (the 3 skips are the real-DEM-only elevation test + the two region-build-dep modules, now `importorskip`'d). What's **still open** and NOT in this PR: real 3DEP DEMs (needs network — P0-c), the real-export spike (needs real files — P0-d), `V1-12` source-archival, and all of P0-f. The quality-bar agreement with Dom below is also still open.
...
- [ ] **P0-f · Hygiene + finish.** Route all finals through the blob seam (stop writing `final_*.png` into `region.dir`) + TTL/eviction for jobs/blobs/uploads; structured logging + job-lifecycle logs (still `traceback.print_exc()` today); a real title/legend/margin + print-correct output (embedded DPI, gamut/bleed). *Roadmap: V1-8, V1-11, V1-10.*
