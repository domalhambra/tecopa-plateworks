...
- **`cache/`** (requests-cache from NHD/3DEP) and **`regions/*/dem.tif`, `final_*.png`, `poster.png`** are gitignored. `region.json` / `overview.png` / `hydro.json` ARE committed; the DEM is regenerated via `region_prep.py`. **Root-level `tune.png` / `comparison.png` were never actually ignored** — this line claimed they were, and 4.8 MB of them sat tracked from 2026-08-13 until they were removed on 2026-08-14. Only `regions/*/tune.png` matches the rule; a tuning image written to the repo root does not.
